...
def load_data():
    ...
    df_loaded = pd.read_csv('/content/google_books_dataset.csv')
...
def load_gemini_model():
    try:
        # In Colab, you'd use userdata.get, but for a deployed Streamlit app,
        # environment variables are more common. Let's use os.getenv for broader compatibility.
        # If running in Colab and using secrets, you might need to adjust this.
        GEMINI_API_KEY = os.getenv('GEMINI_API_KEY')
        if not GEMINI_API_KEY:
            st.warning("GEMINI_API_KEY not found in environment variables. RAG chatbot functionality will be limited.")
            return None
        genai.configure(api_key=GEMINI_API_KEY)
        return genai.GenerativeModel('gemini-pro')
    except Exception as e:
        st.error(f"An error occurred during Gemini API configuration: {e}. RAG chatbot functionality will be limited.")
        return None
...
model_gemini = load_gemini_model()
...
def rag_chatbot(user_query, df_rag, embeddings_rag, model_gemini_rag, st_model_rag, top_k=3):
    if model_gemini_rag is None:
        return "Gemini API not configured. Cannot provide RAG-based response. Please set GEMINI_API_KEY."

    # Step 1: Retrieve relevant documents (book descriptions)
    query_embedding = st_model_rag.encode([user_query])
    scores = cosine_similarity(query_embedding, embeddings_rag)[0]
    top_indices = np.argsort(scores)[::-1][:top_k]

    retrieved_info = []
    for idx in top_indices:
        book = df_rag.iloc[idx]
        retrieved_info.append(f"Title: {book['title']}\nAuthor: {book['authors']}\nDescription: {book['description']}\n")

    # Step 2: Augment the prompt with retrieved information
    context = "Here is some information about books that might be relevant to the user's query:\n\n" + "\n---\n".join(retrieved_info)

    prompt = f"You are an AI Librarian. Use the following book information to answer the user's question. If the information does not contain the answer, state that you don't know, but try to infer from the provided context if possible. Always be helpful and informative.\n\nContext:\n{context}\n\nUser Question: {user_query}\n\nAI Librarian:"

    # Step 3: Generate a response using Gemini
    try:
        response = model_gemini_rag.generate_content(prompt)
...
if menu == "Home":
    ...
    st.write("Ask me anything about books or the library system! (Powered by RAG and Gemini)")
    user_question = st.text_input("Type your question here:")
    if user_question:
        # Use the RAG chatbot with the loaded models and data
        response = rag_chatbot(user_question, df, embeddings, model_gemini, st_model)
        st.info(f"AI Librarian: {response}")
